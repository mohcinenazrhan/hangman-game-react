...
class App extends Component {
	...
	letterClick(letter) {
		...
		if (this.isCompleted(expression)) {
			...
			if (nbrTry >= this.state.stillTry) {
				...
				this.showExpression();
			} else if (nbrTry % this.progressDrawStep === 0 && this.progressDraw + 1 < this.drawSteps)
				this.progressDraw++;
	...
	getKeyboard() {
		return this.state.letters.map((row, index) => {
			const className =
				...
				(row.correct === null ? '' : row.correct ? 'btn-correct' : 'btn-wrong');
			return (
				<button
					className={className}
					key={index}
					disabled={row.disabled}
					onClick={() => this.letterClick(row.letter)}
				>
					{row.letter}
				</button>
			);
   ...
   */
	getBtnNewGame() {
		return (
			<button className="btn btn-control" onClick={this.newGame}>
				{' '}
				REJOUER{' '}
			</button>
		);
			...
			<div className="App">
				...
				<div className="word-container">
					{expression.map((row, index) => (
						<div style={row.style} className={row.class} key={index}>
							{row.hidden === true ? '_' : row.letter}
						</div>
					))}
				</div>
